...
export const qualPay = (formId: string, settings: QualpaySettings, apiClient: ApiClient) => {
	...
	return {
		...
		loadCheckout(
			...
			loadScript(clientSource)
				.then(async function(scriptRef) {
					const response = await apiClient.getCheckoutKey();
					const {transientKey} = response;
					window.qpEmbeddedForm.loadFrame(settings.merchantId,
						{
							paymentFields,
							formId,
							mode: settings.mode,
